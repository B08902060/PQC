...
void tc3_interpolate_neon_SB3(uint16_t *restrict _poly, uint16_t *restrict w)
{
    ...
    for (uint16_t addr = 0; addr < SB3_RES; addr+= 16)
    {
        ...
        vload_x2(tmp1, &_poly[addr + 0*SB3]);
        vadd_x2(r0, tmp1, r0);
        vstore_x2(&_poly[addr + 0*SB3], r0);

        vload_x2(tmp2, &_poly[addr + 1*SB3]);
        vadd_x2(c1, tmp2, c1);
        vstore_x2(&_poly[addr + 1*SB3], c1);

        vload_x2(tmp3, &_poly[addr + 2*SB3]);
        vadd_x2(c2, tmp3, c2);
        vstore_x2(&_poly[addr + 2*SB3], c2);

        vload_x2(tmp1, &_poly[addr + 3*SB3]);
        vadd_x2(c3, tmp1, c3);
        vstore_x2(&_poly[addr + 3*SB3], c3);

        vload_x2(tmp2, &_poly[addr + 4*SB3]);
        vadd_x2(r4, tmp2, r4);
        vstore_x2(&_poly[addr + 4*SB3], r4);
    }
}
...
void tc3_interpolate_neon_SB2(uint16_t *restrict _poly, uint16_t *restrict w)
{
    ...
    for (uint16_t addr = 0; addr < SB2_RES; addr+= 16)
    {
        ...
        vload_x2(tmp1, &_poly[addr + 0*SB2]);
        vadd_x2(r0, tmp1, r0);
        vstore_x2(&_poly[addr + 0*SB2], r0);

        vload_x2(tmp2, &_poly[addr + 1*SB2]);
        vadd_x2(c1, tmp2, c1);
        vstore_x2(&_poly[addr + 1*SB2], c1);

        vload_x2(tmp3, &_poly[addr + 2*SB2]);
        vadd_x2(c2, tmp3, c2);
        vstore_x2(&_poly[addr + 2*SB2], c2);

        vload_x2(tmp1, &_poly[addr + 3*SB2]);
        vadd_x2(c3, tmp1, c3);
        vstore_x2(&_poly[addr + 3*SB2], c3);

        vload_x2(tmp2, &_poly[addr + 4*SB2]);
        vadd_x2(r4, tmp2, r4);
        vstore_x2(&_poly[addr + 4*SB2], r4);
    }
}
...
void tc3_interpolate_neon_SB1(uint16_t *restrict _poly, uint16_t *restrict w)
{
    ...
    for (uint16_t addr = 0; addr < SB1_RES; addr+= 16)
    {
        ...
        vload_x2(tmp1, &_poly[addr + 0*SB1]);
        vadd_x2(r0, tmp1, r0);
        vstore_x2(&_poly[addr + 0*SB1], r0);

        vload_x2(tmp2, &_poly[addr + 1*SB1]);
        vadd_x2(c1, tmp2, c1);
        vstore_x2(&_poly[addr + 1*SB1], c1);

        vload_x2(tmp3, &_poly[addr + 2*SB1]);
        vadd_x2(c2, tmp3, c2);
        vstore_x2(&_poly[addr + 2*SB1], c2);

        vload_x2(tmp1, &_poly[addr + 3*SB1]);
        vadd_x2(c3, tmp1, c3);
        vstore_x2(&_poly[addr + 3*SB1], c3);

        vload_x2(tmp2, &_poly[addr + 4*SB1]);
        vadd_x2(r4, tmp2, r4);
        vstore_x2(&_poly[addr + 4*SB1], r4);
    }
}


void neon_toom_cook_333_combine(uint16_t *restrict _polyC, uint16_t *restrict _polyA, uint16_t *restrict _polyB)
{
    // TC3-3-3 Combine
    uint16_t *aw[5], *bw[5];
    ...
    // Evaluate A, Copy
    // Size: 432 to 144x5
    tc3_evaluate_neon_SB1(aw, _polyA);

    // Evaluate B, Copy
    // Size: 432 to 144x5
    tc3_evaluate_neon_SB1(bw, _polyB);
    ...
    // Transpose 8x8x16
    //MODIFICATION
    batch_mul(tmp_cc, tmp_aa, tmp_bb);
    /*
    ...
    */
    ...
    tc3_interpolate_neon_SB1(_polyC, tmp_cc);
}

void _poly_neon_reduction(uint16_t *_poly, uint16_t *tmp)
{
...
void karat_neon_interpolate_SB0(uint16_t *restrict _poly, uint16_t *restrict w[3])
{
    ...
    for (uint16_t i = 0; i < SB0_RES; i+=32)
    {
        ...
        vload(tmp0, &_poly[0*SB0 + i]);
        vload(tmp1, &_poly[1*SB0 + i]);
        vload(tmp2, &_poly[2*SB0 + i]);
        ...
        vstore(&_poly[0*SB0 + i], tmp0);
        vstore(&_poly[1*SB0 + i], tmp1);
        vstore(&_poly[2*SB0 + i], tmp2);

    }
}

void _poly_mul_neon(uint16_t *restrict _polyC, uint16_t *restrict _polyA, uint16_t *restrict _polyB)
{
    uint16x8x4_t zero;
    ...
    karat_neon_evaluate_SB0(kaw, _polyA);
    // Toom-Cook-3 Evaluate B
    karat_neon_evaluate_SB0(kbw, _polyB);
    ...
    _poly_neon_reduction(_polyC, tmp_ab);
}
...
void poly_Rq_mul_small(poly *r, poly *a, poly *b)
{
    ...
    // Multiplication
    _poly_mul_neon(r->coeffs, a->coeffs, b->coeffs);
